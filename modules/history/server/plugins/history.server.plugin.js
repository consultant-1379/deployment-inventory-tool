...
module.exports = function (ditObject) {
  // Write a log on any object creation/update
  ditObject.pre('save', async function (next) {
    ...
    if (this.name.toUpperCase().includes('SNAPSHOT')) {
      logger.info('no logging for Snapshot artifacts.. returning.');
      return next();
    }
    try {
      if (this.isNew) {
      ...
      var MeanDbSchema = require(`../../../${collectionName}/server/models/${collectionName}.server.model`).Schema; // eslint-disable-line global-require, max-len
  ...
  ditObject.pre('remove', async function (next) {
    var collectionName = this.collection.name;
    var HistoryDbSchema = History.getSchema(collectionName);
    var historyDoc;
    try {
